...
const DataItem: FC<IDataItem> = ({ label, value }) => {
  return (
    <div className="h-[188px] bg-[#262628B2] rounded-lg flex flex-col items-center justify-center lg:h-[109px]">
      <div className="text-xl text-white font-bold mb-2 lg:text-lg">{label}</div>
...
async function getData() {
  // cache for 10s
  const res = await fetch('https://nulscan.io/api/api/nuls/assets/get', { next: { revalidate: 10 } })
  if (!res.ok) {
    return {}
  }
  return res.json()
    ...
    <section className="bg-[url('/home/home5.jpg')] bg-cover bg-center py-[132px] lg:py-16">
      <div className="wrap">
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-1 lg:px-6">
          <DataItem label="Total Supply" value={formatData(data.totalAssets || '')} />
          <DataItem label="Total Staked" value={formatData(data.deposit || '')} />
          <DataItem label="Circulating Supply" value={formatData(data.circulation || '')} />
          <DataItem label="NULS Burned" value={formatData(data.destroy || '')} />
        </div>
